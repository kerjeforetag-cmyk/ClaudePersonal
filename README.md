...
- **Every deck is unique to the customer** — the generator derives a profile from each hub (industry category, situation — greenfield / conversion / refit / OEM, fleet size, sites, country) and composes bespoke copy: a situation-specific headline (a forklift conversion opens "The last battery room you'll ever run", a marine refit "Off diesel. Still on the water", a greenfield warehouse "Zero lead, from day one", an OEM partner "One energy system. Every platform you ship"), stats computed from their own fleet numbers, product cards chosen for their world, and a rollout timeline anchored to their close month. Two customers with the same objective get visibly different presentations, never a mail-merge — and even two customers in the *same* category open differently, thanks to seeded headline/framing pools. A **↻ New take** button recomposes the same facts with a different headline, framing and closing (deterministic per take). Stats show their math ("310 trucks × ~0.3 swaps per shift"), every question the customer asked in the hub is quoted and answered, documents already shared appear as an evidence strip, and the customer's own technical gatekeeper gets a named compliance note.
- **Decks that sell** — a per-SKU "negotiated advantage" chart computed from the hub's price list, reference cases pulled from your other proven hubs, a document reference number + validity date, and your real sender name/workspace; when deck analytics show the customer dwells on pricing, the next deck **automatically leads with the commercial section**
